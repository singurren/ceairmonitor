...
- 为了降低一次打开太多页触发风控的概率，当前会按“本轮待开日期数 + 当前轮询间隔”动态分批开页：
  - 正常轮询时会优先更快消化完当前任务，例如 `13-15` 个日期时按 `5` 页一批、页间隔 `15` 秒、补批间隔 `2` 分钟处理
  - 如果服务端已经因为疑似风控把轮询间隔退避到 `10` 分钟或 `15` 分钟，扩展也会同步降速
  - 例如同样是 `13-15` 个日期：
    - 轮询间隔 `5` 分钟：`5` 页一批，页间隔 `15` 秒，补批间隔 `2` 分钟
    - 轮询间隔 `10` 分钟：`4` 页一批，页间隔 `20` 秒，补批间隔 `3` 分钟
    - 轮询间隔 `15` 分钟及以上：`4` 页一批，页间隔 `25` 秒，补批间隔 `3` 分钟
  - 日期较少时也会自动减少每批开页数，并进一步拉大单页间隔
  - 会按日期从近到远依次打开
  - 这样在日期较少或已进入轮询退避时都会更保守；同时最坏 `15` 个日期任务仍会控制在当前轮询间隔内完成
