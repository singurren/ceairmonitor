...
- 为了降低一次打开太多页触发风控的概率，当前会按本轮待开日期数动态分批开页：
  - `1-2` 个日期：每批 `1` 页，页间隔 `30` 秒，补批间隔 `3` 分钟
  - `3-4` 个日期：每批 `2` 页，页间隔 `25` 秒，补批间隔 `3` 分钟
  - `5-8` 个日期：每批 `3` 页，页间隔 `20` 秒，补批间隔 `2` 分钟
  - `9-12` 个日期：每批 `4` 页，页间隔 `15` 秒，补批间隔 `2` 分钟
  - `13-15` 个日期：每批 `5` 页，页间隔 `15` 秒，补批间隔 `2` 分钟
  - 会按日期从近到远依次打开
  - 这样在日期较少时会减少每批开页数，并拉大单页间隔；最坏 `15` 个日期任务时，最多约 `7` 分钟可以消化完，能覆盖 `10` 分钟一轮的状态刷新
...
- 它的 `batchSize / spacingMs / followupDelayMinutes` 是否符合本轮日期数对应的动态策略
