...
- 只有服务端处于当天值守时段内，扩展才会消费自动开页任务
- 如果 WSL 服务最新状态里出现可兑日期任务，扩展会自动打开对应航班列表页
- 为了降低一次打开太多页触发风控的概率，当前会按“本轮待开日期数 + 当前轮询间隔”动态分批开页：
  - 轮询间隔只按 `10 / 15 / 20 / 25 / 30` 分钟五档处理；不会再出现没有意义的 `5` 分钟档
  - 每个轮询间隔档位下，都会再按日期数分成 `1-2 / 3-4 / 5-8 / 9-12 / 13-15` 五档分别决定：
    - 每批打开多少页
    - 单页间隔多久
    - 下一批延后多久再开
  - 例如同样是 `13-15` 个日期：
    - 轮询间隔 `10` 分钟：`5` 页一批，页间隔 `15` 秒，补批间隔 `2` 分钟
    - 轮询间隔 `15` 分钟：`4` 页一批，页间隔 `20` 秒，补批间隔 `3` 分钟
    - 轮询间隔 `20` 分钟：`4` 页一批，页间隔 `25` 秒，补批间隔 `4` 分钟
    - 轮询间隔 `25` 分钟：`4` 页一批，页间隔 `30` 秒，补批间隔 `5` 分钟
    - 轮询间隔 `30` 分钟：`3` 页一批，页间隔 `35` 秒，补批间隔 `6` 分钟
  - 日期较少时也会自动减少每批开页数，并进一步拉大单页间隔
  - 会按日期从近到远依次打开
  - 这样在日期较少或轮询退避变大时都会更保守；同时最坏 `15` 个日期任务仍会控制在对应轮询间隔内完成
...
- 如果自动打开的页面在 `2` 分钟内还没有回传航班结果，扩展会自动关闭该标签页，并向服务端上报一条风控告警；服务端会推送：
  - `可能已经触发风控，请检查确认`
  - 这类风控告警现在只会发给 `maintainer_serverchan_sendkeys`
- 同一日期任务不是“永远只开一次”
- 扩展会按“最近一次成功轮询结果”去重：
  - 同一轮成功轮询里，同一日期任务只开一次
...
- 它的 `effectivePollIntervalSeconds / planLabel / batchSize / spacingMs / followupDelayMinutes` 是否符合本轮日期数和轮询退避档位对应的动态策略
